...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function drawMatrix() {
    ...
    for (let i = 0; i < drops.length; i++) {
      ...
      gradient.addColorStop(0, "#5a0070");
  ...
  class Particle {
    constructor() {
      ...
      this.color = `rgba(${Math.floor(
        Math.random() * 50 + 40
      )}, 0, ${Math.floor(Math.random() * 70 + 60)}, ${
        Math.random() * 0.5 + 0.3
      })`;
    }
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      for (let j = i; j < particles.length; j++) {
        ...
        if (distance < 150) {
          ...
          particlesCtx.strokeStyle = `rgba(58, 0, 96, ${0.3 - distance / 500})`;
